...
ProactorEventLoop). pgvector type adapters are registered on every
fresh connection via the pool's ``init`` callback so vector columns
come back as numpy arrays automatically. Even though v1 of this app
doesn't use vectors, the codec is set up now so adding semantic
features later is a non-event.

Module-level state is intentional: there's exactly one pool per
...
async def _init_connection(conn: asyncpg.Connection) -> None:
    """Run once per fresh connection -- registers pgvector types."""
