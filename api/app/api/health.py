...
async def health() -> HealthResponse:
    ...
    Always returns 200 so a load balancer doesn't kill the pod just
    because the database is briefly unreachable. The frontend reads
    ``db`` to decide whether to show a degraded banner.
    """
    try:
        ok = await db.ping()
